...
├── scripts/
│   ├── CardData.gd                    ← 数据模型（13 个属性，含族/化合价/元素类型）
│   ├── CardDatabase.gd                ← 牌库（18 种 × 6 张 = 108 张，Fisher-Yates）
│   ├── GameManager.gd                 ← 规则引擎（牌权轮转、族炸接炸链、化合物校验）
│   ├── GameUI.gd                      ← UI 控制器（手牌渲染、着色、步骤流、AI）
│   └── Utils.gd                       ← 工具函数（detect_pattern、get_compound_formula、compare_cards）
├── CHEMICAL_COLLISION_GAME.md         ← 游戏设计文档
├── GAMEPLAY_RULES.md                  ← 玩法规则与程序实现文档
├── ARCHITECTURE.md                    ← 本文档（程序架构与实现）
...
| `atomic_number` | int | L39 | 原子序数 1–18 |
| `group` | String | L40 | 族 "IA"/"0"/... |
| `period` | int | L41 | 周期 1–3 |
| `element_type` | String | L42 | "金属"/"非金属"/"准金属"/"稀有气体" |
| `common_valence` | Array | L45 | 化合价列表 e.g. `[1]` `[-2,4,6]` |
| `atomic_weight` | float | L47 | 相对原子质量，用于排序平局 |

**16 族常量** (L7–L22): IA / IIA / IIIA / IVA / VA / VIA / VIIA / 0 / IIIB–VIII / IB / IIB
...
**文件**: `scripts/CardDatabase.gd`（83 行）

**职责**: 生成 108 张牌的牌库，提供洗牌和抽牌功能。

**数据结构**:
```gdscript
# 18 种元素的原始数据
static func get_element_data() -> Array  # L9  — 18 条：symbol / cn / en / ... / 化合价 / 原子量 / 描述

var deck: Array = []                     # L36 — 当前牌库
const COPIES_PER_ELEMENT = 6             # L33
const TOTAL_CARDS = 108                  # L34
```

**关键方法**:
| 方法 | 位置 | 说明 |
|------|------|------|
| `generate_deck()` | L40 | 18 种 × 6 = 108 张 |
| `shuffle()` | L54 | Fisher-Yates 洗牌 |
| `draw_card()` | L63 | 弹出栈顶 |
| `draw_cards(n)` | L70 | 批量抽牌 |
...
**文件**: `scripts/GameUI.gd`（648 行）

**职责**: 页面管理、手牌渲染、出牌步骤流、AI 候选、状态显示。
...
| 函数 | 位置 | 说明 |
|------|------|------|
| `_setup_pages()` | L57 | 绑定所有按钮 + SpinBox + 退出按钮 |
...
| 函数 | 位置 | 说明 |
|------|------|------|
| `_format_player_status()` | L162 | 圆形状态：● 玩家1 ◄ → ● AI1 ❄ → ● AI2 ⏸ |

**手牌渲染** (`_update_hand_buttons()`, L297–335):
...
着色规则 (_get_card_color, L251):
  H 浅蓝 / O 蓝色 / N 蓝紫 / C,B,Si,S 黄色 / P 红白 / 卤族绿色
  其余金属灰 / 非金属/准金属绿 / 稀有气体白
```

**出牌步骤流** (`_update_action_panel()`, L235–C240):
...
**基化合物确认** (`_on_confirm_compound()`, L443):
...
├── HelpPage                         ← 帮助页
│   ├── HelpTitle                    ← "帮助与规则"
│   ├── HelpContent                  ← 完整游戏规则（中文）
│   └── HelpBackBtn                  ← "返回"
├── GamePage                         ← 游戏页
│   ├── TitleLabel                   ← "化学碰撞"
│   ├── InfoLabel                    ← 圆形状态
│   ├── TableLabel                   ← 桌面牌型
│   ├── HandContainer (Flow)         ← 手牌按钮
...
```
_refresh_ui()
  ├── InfoLabel ← _format_player_status()
...
人类操作:
  ...
  跳过 → player_pass(idx) → _refresh_ui()
...
玩家 C 过 → player_pass(C) → mark passed → 抽牌
玩家 D 过 → player_pass(D) → mark passed → 抽牌
玩家 A 过 → player_pass(A) → mark passed → 抽牌
...
get_compound_formula(cards, custom_valences):
  ...
  // Step 1: 统计元素
  elem_counts = {Na:1, Cl:1, ...}

  // Step 2: 正负价分类
  pos_list: [{symbol, valence, count}, ...]   ← metal first
